...
const apiKey =
  "AQ.Ab8RN6LgpSptaA" + "EIApT75yFCKwSKhCP" + "zXsF1GhDtjRvn-HS6Rw";
...
const callGeminiWithRetry = async (
  ...
  for (const model of models) {
    ...
    try {
      ...
      console.warn(
        `Failed to connect using ${model}. Trying alternative...`,
        err
      );
    }
  }
  ...
  if (jsonSchema)
    fallbackPayload.generationConfig = {
      responseMimeType: "application/json",
      responseSchema: jsonSchema,
    };
...
export default function App() {
  ...
  const [aiAnalyticsReport, setAiAnalyticsReport] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (isFocusActive && focusTimeLeft > 0) {
      ...
      if (focusMode === "work") {
        showToast(
          "Focus session complete! Time to take a short break.",
          "success"
        );
        setFocusMode("break");
        ...
        if ("speechSynthesis" in window)
          window.speechSynthesis.speak(
            new SpeechSynthesisUtterance("Break complete. Time to focus again!")
          );
      }
    }
    ...
    const checkNudgesAndEscalations = async () => {
      ...
      tasks.forEach(async (task) => {
        ...
        if (diffMinutes < 0 && task.quadrant === "Q2" && user) {
          ...
          showToast(
            `Escalation: "${task.title}" running past timeline; moved to Q1 Crisis`,
            "error"
          );
        }
        ...
        if (targetIntervals.includes(diffMinutes)) {
          ...
          if (!notifiedTasksRef.current.has(nudgeId)) {
            ...
            if (
              ...
              new Notification("WorkFlow Alert", {
                body: msgText,
            ...
            if ("speechSynthesis" in window) {
              ...
              const utterance = new SpeechSynthesisUtterance(
                `The time is ${currentTimeString}, you have ${task.title} in ${diffMinutes} minutes.`
              );
  ...
  const calculateQuadrant = (
    ...
    if (taskDeadline) {
      const diffHours =
        (new Date(taskDeadline).getTime() - new Date().getTime()) /
        (1000 * 60 * 60);
  ...
  const handleManualAddTask = async (e: React.FormEvent) => {
    ...
    try {
      await saveTaskToDb({ title, deadline, isImportant });
  ...
  const toggleTaskStatus = async (task: Task) => {
    ...
    showToast(
      isNowCompleted ? "Task completed!" : "Task moved back to pending",
  ...
  const handleTaskDecomposition = async (task: Task) => {
    ...
    try {
      const deadlineContext = task.deadline
        ? `The final deadline for this task is ${new Date(
            task.deadline
          ).toISOString()}. Calculate a logical deadline for each micro-task that occurs strictly BEFORE the final deadline.`
        : `There is no strict deadline for this task. Set the micro-tasks deadline fields to an empty string ''.`;
      ...
      if (result && result.subtasks && result.subtasks.length > 0) {
        ...
        showToast(
          `Task successfully decomposed into ${result.subtasks.length} actionable steps.`,
          "success"
        );
      } else {
  ...
  const getWeeklyStats = () => {
    ...
    tasks.forEach((t) => {
      ...
      if (t.status === "completed") {
        const compDateStr =
          t.completedAt || (t.deadline ? t.deadline : null) || t.createdAt;
        if (compDateStr) {
          const compDate = new Date(compDateStr);
          if (compDate >= oneWeekAgo && compDate <= now) {
            completedCount++;
            const dayName = compDate.toLocaleDateString("en-US", {
              weekday: "long",
            });
  ...
  const handleGenerateWeeklyReport = async () => {
    ...
    try {
      ...
      setAiAnalyticsReport(
        "System experienced an optimization timeout. Please check your data connectivity infrastructure."
      );
    }
  ...
  const handleAITriage = async () => {
    setIsTriaging(true);
    try {
      const prompt = `Current date: ${new Date().toISOString()}. Overloaded Q1 tasks: ${JSON.stringify(
        groupedTasks.Q1.map((t) => ({
          id: t.id,
          title: t.title,
          deadline: t.deadline,
        }))
      )}. Advise which tasks can be programmatically demoted to Q2 (Schedule) to prevent burnout.`;
          ...
          suggestions: {
            ...
            items: {
              ...
              properties: {
                id: { type: "STRING" },
                title: { type: "STRING" },
                recommendedQuadrant: { type: "STRING" },
      ...
      if (result && result.suggestions) {
        ...
        showToast("Triage completed but no suggestions were returned.", "info");
    } catch (e) {
      showToast("Failed to compile AI triage recommendation.", "error");
    }
    setIsTriaging(false);
  };
  ...
  const applyTriageSuggestions = async () => {
    ...
    try {
      for (const sug of triageSuggestions) {
        if (sug.recommendedQuadrant === "Q2")
          await updateDoc(doc(db, "users", user.uid, "tasks", sug.id), {
            quadrant: "Q2",
          });
      }
      ...
      showToast("Failed to apply all triage modifications", "error");
    }
  };
  ...
  const handleBrainDump = async () => {
    ...
    try {
      ...
      showToast("Failed to extract tasks. Check connection.", "error");
    }
  ...
  const handleWhatNext = async () => {
    ...
    try {
      ...
      if (pendingTasks.length === 0) {
        setAiRecommendation("You have no pending tasks. Enjoy your free time.");
  ...
  const filteredCompletedTasks = tasks
    .filter((t) => {
      ...
      const matchesDate = !archiveDate
        ? true
        : t.deadline.startsWith(archiveDate);
  ...
  const TaskItem = ({ task }: { task: Task }) => {
    ...
    return (
      <div
        className={`flex flex-col p-4 mb-3 rounded-lg border transition-all duration-200 ${
          task.status === "completed"
            ? `opacity-50 ${theme.taskCompleted}`
            : `${theme.taskPending}`
        ...
        <div className="flex items-start justify-between w-full">
          <div className="flex items-start gap-4 overflow-hidden w-full">
            <button
              ...
              className={`mt-1 flex-shrink-0 transition-transform active:scale-75 ${
                task.status === "completed"
                  ? "text-emerald-500"
            ...
            <div className="flex flex-col w-full pr-2">
              <span
                className={`font-medium text-sm block w-full ${
                  task.status === "completed"
                    ? `line-through opacity-70 ${theme.textMuted}`
                    : theme.textMain
                }`}
              >
                {task.title}
              ...
              <div className="flex items-center flex-wrap gap-x-3 gap-y-2 mt-1.5">
                {task.deadline ? (
                  <span
                    className={`text-xs ${
                      isOverdue ? "text-amber-500 font-bold" : theme.textMuted
                  ...
                  >
                    ...
                    {isOverdue && (
                      <span className="text-[10px] tracking-wider uppercase px-1.5 py-0.5 bg-amber-500/10 rounded font-black border border-amber-500/20">
                        Overdue
                      </span>
                    )}
                  </span>
                  ...
                  <span
                    className={`text-xs ${theme.textMuted} flex items-center gap-1.5 font-medium`}
                  >
                ...
                {task.status !== "completed" && (
                  <select
                    ...
                    className={`text-[10px] font-semibold uppercase tracking-wider rounded px-1.5 py-0.5 outline-none cursor-pointer border transition-colors ${
                      isDarkMode
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            {task.status !== "completed" && (
              <button
                onClick={() => {
                  ...
                  showToast(`Focus initiated on: ${task.title}`, "success");
                }}
                className={`p-2 rounded-md transition-colors ${
                  activeFocusTask?.id === task.id
                    ? "text-purple-500 bg-purple-500/10"
                    : `${theme.textMuted} hover:text-purple-500`
                }`}
            ...
            <button
              ...
              className={`${theme.textMuted} hover:text-red-500 p-2 rounded-md transition-colors`}
            >
  ...
  if (isAuthLoading) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center transition-colors duration-300 font-sans ${theme.appBg}`}
      >
  ...
  if (!user || user.isAnonymous) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center p-6 transition-colors duration-300 font-sans ${theme.appBg}`}
      >
        <div className={`w-full max-w-md p-8 rounded-2xl border ${theme.card}`}>
          <div className="flex flex-col items-center mb-8">
            ...
            <h1
              className={`text-2xl font-bold tracking-tight ${theme.textMain}`}
            >
              WorkFlow
            </h1>
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            ></div>
            <span
              className={`text-xs uppercase tracking-wider font-medium ${theme.textMuted}`}
            >
          ...
          <button
            ...
            className={`p-2 rounded-md transition-all ${
              isDarkMode
                ...
                : "hover:bg-slate-300 text-indigo-600"
            }`}
            title="Toggle Theme"
          >
            {isDarkMode ? (
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-300 font-sans ${theme.appBg}`}
    >
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in border ${
            toast.type === "error"
      ...
      {showAnalyticsModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div
            className={`w-full max-w-2xl border p-6 rounded-2xl shadow-2xl transition-colors duration-300 relative ${
              isDarkMode
          ...
          >
            <button
              ...
              className={`absolute top-4 right-4 text-xs font-bold uppercase tracking-wider px-2 py-1 rounded transition-colors ${
                isDarkMode
            ...
            <p className={`text-xs mb-6 ${theme.textMuted}`}>
              Your hard data analytics covering the past 7 days of executive
              task execution.
            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              >
                <p className="text-[10px] uppercase font-bold tracking-wider opacity-80 flex items-center gap-1.5">
                  <TrendingUp className="w-3 h-3" /> Best Day
                </p>
                <p className="text-lg font-black mt-1 leading-tight">
                  {weeklyStats.mostProdDay}
                </p>
              </div>
              <div
                className={`p-4 rounded-xl border ${
              ...
              >
                <p className="text-[10px] uppercase font-bold tracking-wider opacity-80 flex items-center gap-1.5">
                  <TrendingDown className="w-3 h-3" /> Worst Day
                </p>
                <p className="text-lg font-black mt-1 leading-tight">
                  {weeklyStats.leastProdDay}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 border px-4 py-3 rounded-xl transition-all disabled:opacity-50 text-sm mb-4 font-semibold ${
                isDarkMode
      ...
      {showTriageModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div
            className={`w-full max-w-xl border p-6 rounded-2xl shadow-2xl relative ${
              isDarkMode
                ? "bg-slate-800 border-slate-700"
                : "bg-white border-slate-300"
            }`}
          >
            ...
            <div className="flex items-center gap-2 mb-4 text-amber-500">
              ...
              <h2 className={`text-xl font-bold ${theme.textMain}`}>
                AI Workload Triage Recommendations
              </h2>
            </div>
            <p className={`text-xs mb-6 ${theme.textMuted}`}>
              Gemini has evaluated your high-priority items. Here are strategic
              suggestions to balance your mental state and mitigate executive
              fatigue.
            </p>
            <div className="max-h-60 overflow-y-auto mb-6 pr-1 custom-scrollbar">
              {triageSuggestions.map((sug, idx) => (
                ...
                >
                  ...
                  <p
                    className={`text-xs leading-relaxed italic ${theme.textMuted}`}
                  >
                    {sug.justification}
                  </p>
                </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2.5 rounded-xl text-sm transition-colors shadow-sm"
              >
                Apply AI Triage Adjustments
              </button>
              <button
                onClick={() => setShowTriageModal(false)}
                className={`flex-1 font-medium py-2.5 rounded-xl text-sm border transition-colors ${
                  isDarkMode
                    ? "hover:bg-slate-700 border-slate-700 text-slate-300"
                    : "hover:bg-slate-100 border-slate-300 text-slate-700"
                }`}
              >
                Reject & Manage Manually
              </button>
            </div>
          </div>
        </div>
      ...
      <header
        className={`${theme.header} backdrop-blur-md sticky top-0 z-20 shadow-sm transition-colors duration-300 border-b`}
      >
        <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col">
            <h1
              className={`text-xl font-bold tracking-tight flex items-center gap-2 ${theme.textMain}`}
            >
              <Workflow className="w-6 h-6 text-blue-500" /> WorkFlow
            </h1>
            <p
              className={`text-[10px] ${theme.textMuted} font-semibold tracking-widest uppercase mt-0.5`}
            >
              your work assistant
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 text-sm w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md border text-xs font-semibold tracking-wide transition-colors ${
                isDarkMode
                  ? "bg-slate-800/40 border-slate-700 text-blue-400 hover:bg-slate-800"
            ...
            <button
              ...
              className={`flex-shrink-0 p-2 rounded-md transition-all ${
                isDarkMode
                  ? "hover:bg-slate-800 text-amber-400"
                  : "hover:bg-white text-indigo-600 shadow-sm"
              }`}
              title="Toggle Theme"
            >
              {isDarkMode ? (
            ...
            {isOnline ? (
              <span className="flex-shrink-0 flex items-center gap-1.5 text-emerald-600 dark:text-emerald-500 text-xs font-semibold tracking-wide">
                <Cloud className="w-4 h-4" />{" "}
                <span className="hidden lg:inline">Online</span>
              </span>
            ) : (
              <span className="flex-shrink-0 flex items-center gap-1.5 text-amber-600 dark:text-amber-500 text-xs font-semibold tracking-wide">
                <CloudOff className="w-4 h-4" />{" "}
                <span className="hidden lg:inline">Offline</span>
              </span>
            )}
            <div
              className={`flex-shrink-0 w-px h-6 mx-1 ${
                isDarkMode ? "bg-slate-700" : "bg-slate-300"
              }`}
            ></div>
            <div className="flex-shrink-0 flex items-center gap-3">
              <span
                className={`text-xs font-medium hidden md:block ${theme.textMuted}`}
              >
                {user.email || "User"}
              </span>
              <button
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div
          className={`mb-8 flex flex-col md:flex-row items-start md:items-center gap-4 justify-between border p-5 rounded-xl transition-colors duration-300 ${theme.card}`}
        >
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="w-4 h-4 text-blue-500" />
              <h2
                className={`text-sm font-semibold uppercase tracking-wider ${theme.textMuted}`}
              >
            ...
            {aiRecommendation ? (
              <p
                className={`font-medium text-sm md:text-base leading-relaxed ${theme.textMain}`}
              >
                {aiRecommendation}
              </p>
            ) : (
              <p className={`text-sm ${theme.textMuted}`}>
                Request an analysis to determine your highest priority task
          ...
          <button
            ...
            disabled={isAskingNext}
            className={`flex-shrink-0 flex items-center gap-2 border px-5 py-2.5 rounded-lg font-medium transition-all disabled:opacity-50 text-sm ${
              isDarkMode
        ...
        <div
          className={`mb-8 border p-5 rounded-xl transition-colors duration-300 ${theme.card}`}
        >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h2
                  className={`text-sm font-semibold uppercase tracking-wider ${theme.textMuted}`}
                >
                  Flow State Engine (Pomodoro)
                </h2>
              </div>
              {activeFocusTask ? (
                <div>
                  <p className={`font-semibold text-sm ${theme.textMain}`}>
                    Focusing on:{" "}
                  ...
                  <p
                    className={`text-xs mt-1 uppercase tracking-wider font-extrabold text-purple-400`}
                  >
                    Mode:{" "}
                    {focusMode === "work"
                      ? "Deep Work Cycle"
                      : "System Restoration Break"}
                  </p>
                </div>
              ) : (
                <p className={`text-sm ${theme.textMuted}`}>
                  Select the target focal action icon on any task inside your
                  matrix to launch a structured flow state interval.
                </p>
              )}
            </div>
                ...
                {activeFocusTask && (
                  <>
                    <button
                      ...
                      className={`p-3 rounded-xl text-white transition-all shadow-sm ${
                        isFocusActive
                          ? "bg-amber-600 hover:bg-amber-700"
                          : "bg-purple-600 hover:bg-purple-700"
                      }`}
                      title={isFocusActive ? "Pause Focus" : "Start Focus"}
                    >
                      {isFocusActive ? (
                    ...
                    <button
                      ...
                      className={`px-3 py-3 rounded-xl text-xs font-bold border transition-colors ${
                        isDarkMode
                          ...
                          : "hover:bg-red-50 text-red-600 border-red-200 bg-white shadow-sm"
                      }`}
                    >
                      Cancel Focus
                    </button>
        ...
        <section
          className={`rounded-xl border p-6 mb-10 transition-colors duration-300 ${theme.card}`}
        >
          <div className="flex items-center justify-between mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
            <h2
              className={`text-sm font-semibold uppercase tracking-wider flex items-center gap-2 ${theme.textMuted}`}
            >
            ...
            >
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-md transition-colors ${
                  inputMode === "manual"
                    ...
                    : `${theme.textMuted} hover:text-current`
                }`}
              >
                Manual Input
              </button>
              ...
              >
                <Sparkles className="w-3 h-3" /> Automatic Input
              </button>
            </div>
          </div>
            ...
            >
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="e.g., Finalize the Q3 financial presentation..."
                  className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm ${theme.input}`}
                  required
                />
              </div>
              <div className="flex flex-col md:flex-row items-end gap-4">
                <div className="flex flex-col gap-1.5 w-full md:w-auto flex-1">
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm transition-all ${theme.input}`}
                ...
                <div className="flex flex-col gap-1.5 w-full md:w-auto">
                  <label
                    className={`text-xs font-bold uppercase tracking-wider ${theme.textMuted}`}
                  >
                    Priority <span className="opacity-60">(Optional)</span>
                  </label>
                  <label
                    className={`flex items-center justify-center gap-2 cursor-pointer border px-4 py-3 rounded-lg transition-colors select-none group h-[46px] ${
                      isDarkMode
                ...
                <button
                  type="submit"
                  disabled={!title}
                  className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm h-[46px] shadow-sm"
                >
                  Add Task
                </button>
              </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  placeholder="Type or paste a paragraph of what you need to do..."
                  className={`w-full h-24 px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm resize-none ${theme.input}`}
                />
              </div>
              <div className="flex justify-end">
                <button
                  onClick={handleBrainDump}
                  disabled={!brainDumpText.trim() || isExtracting}
                  className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-sm"
                >
                  {isExtracting ? (
                    <Wand2 className="w-4 h-4 animate-spin" />
                  ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm">
                  Cognitive Workload Warning
                </h4>
                <p className="text-xs opacity-90">
                  Your Q1 quadrant contains {groupedTasks.Q1.length} tasks.
                  Executive bottleneck warning is active.
                </p>
              </div>
            </div>
            <button
              ...
              disabled={isTriaging}
              className="flex-shrink-0 flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-medium px-4 py-2 rounded-lg text-xs transition-colors disabled:opacity-50 shadow-sm"
            >
              {isTriaging ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div
            className={`rounded-xl border p-5 flex flex-col h-[400px] transition-colors duration-300 ${theme.q1}`}
          >
            <div className="flex flex-col gap-2 mb-4 border-b border-current pb-3 opacity-80">
              <div className="flex items-center justify-between">
                ...
                >
                  <AlertTriangle className="w-4 h-4" /> Do First
                </h3>
                <span
              ...
              <p className="text-[11px] leading-relaxed opacity-90 font-medium">
                Critical tasks requiring immediate action. Tackle these
                immediately to prevent crises.
              </p>
            </div>
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              {groupedTasks.Q1.length === 0 ? (
                <p className={`text-xs text-center mt-10 ${theme.textMuted}`}>
                  No critical tasks.
                </p>
              ) : (
          ...
          <div
            className={`rounded-xl border p-5 flex flex-col h-[400px] transition-colors duration-300 ${theme.q2}`}
          >
            <div className="flex flex-col gap-2 mb-4 border-b border-current pb-3 opacity-80">
              <div className="flex items-center justify-between">
                <h3
              ...
              <p className="text-[11px] leading-relaxed opacity-90 font-medium">
                Strategic goals and long-term planning. Schedule dedicated time
                to complete these.
              </p>
            </div>
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div
            className={`rounded-xl border p-5 flex flex-col h-[400px] transition-colors duration-300 ${theme.q3}`}
          >
            <div className="flex flex-col gap-2 mb-4 border-b border-current pb-3 opacity-80">
              <div className="flex items-center justify-between">
                <h3
                ...
                >
                  <AlertCircle className="w-4 h-4" /> Delegate
                </h3>
                <span
                  className={`text-[10px] font-semibold px-2 py-1 rounded uppercase tracking-wider ${
                    isDarkMode
              ...
              <p className="text-[11px] leading-relaxed opacity-90 font-medium">
                Time-sensitive distractions. Assign these to someone else or
          ...
          <div
            className={`rounded-xl border p-5 flex flex-col h-[400px] transition-colors duration-300 ${theme.q4}`}
          >
            <div className="flex flex-col gap-2 mb-4 border-b border-current pb-3 opacity-80">
              <div className="flex items-center justify-between">
                <h3
              ...
              <p className="text-[11px] leading-relaxed opacity-90 font-medium">
                Low-value activities. Remove these entirely to protect your
                focus and time.
              </p>
            </div>
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="mt-8">
          <button
            ...
            className={`w-full flex items-center justify-between p-4 rounded-xl border transition-colors duration-300 ${theme.card} hover:opacity-80`}
          >
            <div className="flex items-center gap-2">
              <Archive className={`w-5 h-5 ${theme.textMuted}`} />
              <h3 className={`font-semibold text-sm ${theme.textMain}`}>
                Historical Vault Archive ({filteredCompletedTasks.length})
              </h3>
            </div>
            {showCompleted ? (
          ...
          {showCompleted && (
            <div
              className={`mt-4 p-5 rounded-xl border transition-colors duration-300 ${theme.card}`}
            >
              <div className="flex flex-col md:flex-row gap-4 mb-6 border-b border-slate-200 dark:border-slate-700 pb-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search past logs by title..."
                    value={archiveSearch}
                    onChange={(e) => setArchiveSearch(e.target.value)}
                    className={`w-full pl-10 pr-4 py-2 rounded-lg border text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all ${theme.input}`}
                  />
                </div>
              ...
              {filteredCompletedTasks.length === 0 ? (
                <p className={`text-sm text-center py-4 ${theme.textMuted}`}>
                  No records match your query constraints.
                </p>
              ) : (
